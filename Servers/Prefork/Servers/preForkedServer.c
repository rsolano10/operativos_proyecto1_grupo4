...
/* process the file request clicked in the server */
void parse_request1(int fd, http_request *req)
{
    rio_t rio;
    char buf[MAXLINE], method[MAXLINE], uri[MAXLINE];
...
void process(int fd, struct sockaddr_in *clientaddr, int pid)
{
    int flag = 0;
    ...
    if (flag == 1 && pid == 0)
    {
        ...
        writeFile("Process Killed, pid: ", id);
        kill(id, SIGKILL);
        kill(id + 1, SIGKILL);
    }
}
...
int main()
{
    ...
    data = &config;
    read_config_file(data,PREFORK);
    struct sockaddr_in clientaddr;
    int default_port = data->port;
    int listenfd, connfd;
    int proCount = 2;
    ...
    pid_t pid;
    cont= mmap(NULL, 0x00001000, PROT_READ | PROT_WRITE,  //mmap for manage a global variable between different forks
                    MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    cont[0]=0;
    while (1)
    {
        while (cont[0] < proCount)
        {
