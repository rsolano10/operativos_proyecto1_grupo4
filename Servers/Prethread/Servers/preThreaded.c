...
void *process(void *arg)
{
    ...
    printf("1. accept request as, fd is %d, pid is %d\n", fd, getpid());
    http_request req;
    ...
    if (ffd <= 0)
    {
        ...
        printf("5 Thread terminated, pid: %ld\n", pthread_self());
        writeFile("Thread terminated, pid: ", pthread_self());
        pthread_exit(NULL);
...
int main()
{
    ...
    data = &config;
    read_config_file(data,PRETHREAD);
    struct sockaddr_in clientaddr;
    ...
    int i = 0;
    int flag=0;
    used=malloc(threads*sizeof(int));
    while (1)
    {
        while(!flag){
            for(i=0; i<threads; i++){
        ...
        data.conn=connfd;
        data.index=i;
        if (pthread_create(&tid[i], NULL, process, &data) != 0){
